...
const TERRAIN_WEIGHTS: Dictionary[int, float] = {
...
@onready var costs_tile_layer: TileMapLayer = %CostsTileLayer
@onready var directions_tile_layer: TileMapLayer = %DirectionsTileLayer
...
func get_tile_type_from_cell_position(cell_pos: Vector2i) -> int:
	return main_tile_layer.get_cell_atlas_coords(cell_pos).x


## Convert a cost value into atlas coordinates within the gradient tileset.
func get_tileset_atlas_pos_for_cost_value(cost: float) -> Vector2i:
	return Vector2i(clampi(cost, 0, MAX_VISUAL_COST), 0)
...
## Index 1 = cost map, 2 = direction map
func on_visualization_selection_item_selected(index: int) -> void:
	var vis_layers: Array[TileMapLayer] = [costs_tile_layer, directions_tile_layer]
	for i in vis_layers.size():
		var layer := vis_layers[i]
		layer.set_enabled(index - 1 == i)
